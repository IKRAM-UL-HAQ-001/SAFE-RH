...
class DetailsCategoryFive extends StatefulWidget {

  final Id;
  final Date;
  DetailsCategoryFive({this.Id,this.Date});
  @override
  State<DetailsCategoryFive> createState() => _DetailsCategoryFiveState();
}
class _DetailsCategoryFiveState extends State<DetailsCategoryFive> {
  var loading=true;
  var test=[];
  // var testName;
  var detaillist = [];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        appBar: AppBar(title: Text("Lab Test Detail",style: TextStyle(color: Colors.black,fontSize: 20.0,fontWeight: FontWeight.bold),),centerTitle: true,backgroundColor: Colors.white,automaticallyImplyLeading: false,),
        body:loading?
        Column(
  ...
  Widget getAllAppointments(final_array){
    return Container(
      ...
      child:ListView.builder(
         ...
         itemCount:final_array.length-1,
         itemBuilder: (_, index) => Card(
           margin: const EdgeInsets.all(10),
           child: InkWell(
             child: ListTile(
               title: Text("${final_array[index]['value']}",
                style: TextStyle(color: Colors.black,fontSize: 18.0,fontWeight: FontWeight.bold)
               ),
               subtitle: Text("${final_array[index+1]['value']}",
